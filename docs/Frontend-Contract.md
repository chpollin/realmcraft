...
- `[data-testid="history-entry"]` × `state.historie.length` (chronologisch, eine Karte je Jahreszeit), nach Kapitel gruppiert mit **einem** Gruppenkopf „Kapitel N"; „Gegenwart" nur am jüngsten Eintrag. Trägt ein Eintrag `bild`, folgt `[data-testid="ereignis-bild"][data-jahre]` (`<img>`) plus `[data-testid="generate-ereignisbild"]` (Button, Text-zu-Bild). `[data-testid="faehigkeit"]` × n. (Der frühere „Verlauf dieser Partie" / `chronik-flow` entfällt; Besitz lebt in der Lebenswelt.)
